...
public class WavySpriteEditor : Editor
{
    ...
    void OnSceneGUI()
    {
        ...
        if (Tools.current == Tool.None && pivotMoveMode && script.isActiveAndEnabled)
        {
            ...
            EditorGUI.BeginChangeCheck();
            var fmh_305_61_638340129297954641 = Quaternion.identity; pivotStart = Handles.FreeMoveHandle(pivotStart, HandleUtility.GetHandleSize(pivotStart) * 0.1f, Vector3.zero, Handles.CircleHandleCap);
