...
public class ColliderFixed : MonoBehaviour
{
    ...
    public void SetFixed()
    {
        ...
        m_childOffset = m_baseCenter - (Vector2)transform.position;
        m_childOffset.x /= transform.localScale.x;
        m_childOffset.y /= transform.localScale.y;

        switch (m_fixedX)
        {
            case DirectionX.Left:
                m_childOffset += Vector2.left * (m_baseCollider.bounds.size.x / 2 + m_fixedOffsetX) / transform.localScale.x;
                break;
            case DirectionX.Right:
                m_childOffset += Vector2.right * (m_baseCollider.bounds.size.x / 2 + m_fixedOffsetX) / transform.localScale.x;
                break;
        }
        switch (m_fixedY)
        {
            case DirectionY.Up:
                m_childOffset += Vector2.up * (m_baseCollider.bounds.size.y / 2 + m_fixedOffsetY) / transform.localScale.y;
                break;
            case DirectionY.Down:
                m_childOffset += Vector2.down * (m_baseCollider.bounds.size.y / 2 + m_fixedOffsetY) / transform.localScale.y;
                break;
        }
        ...
        foreach (Collider2D ColliderChild in m_colliderChild)
        {
            ...
            if (m_fixed == FixedOption.BorderOut)
            {
                ...
                {
                    case DirectionX.Left:
                        ColliderChild.offset += Vector2.left * (ColliderChild.bounds.size.x / 2) / transform.localScale.x;
                        break;
                    case DirectionX.Right:
                        ColliderChild.offset += Vector2.right * (ColliderChild.bounds.size.x / 2) / transform.localScale.x;
                        break;
                }
                switch (m_fixedY)
                {
                    case DirectionY.Up:
                        ColliderChild.offset += Vector2.up * (ColliderChild.bounds.size.y / 2) / transform.localScale.y;
                        break;
                    case DirectionY.Down:
                        ColliderChild.offset += Vector2.down * (ColliderChild.bounds.size.y / 2) / transform.localScale.y;
                        break;
                }
            }
            else
